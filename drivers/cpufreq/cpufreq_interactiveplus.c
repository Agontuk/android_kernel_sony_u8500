 ...
 *
 * InteractivePlus governor
 * Interactive governor with hotplugging
 *
...
static int cpufreq_governor_interactiveplus(struct cpufreq_policy *policy,
		unsigned int event);

static struct early_suspend early_suspend;
static unsigned int cpufreq_lcd_status;
static unsigned long current_timer_rate;
...
static void cpufreq_interactiveplus_timer_resched(
...
static unsigned int choose_freq(
	struct cpufreq_interactiveplus_cpuinfo *pcpu, unsigned int loadadjfreq)
{
...
static int cpufreq_interactiveplus_speedchange_task(void *data)
{
	...
	while (1) {
		...
		for_each_cpu(cpu, &tmp_mask) {
		    ...
		    if (num_online_cpus() == 1 && cpufreq_lcd_status == 1) {
                    cpu_up(1);
		    } else if (num_online_cpus() == 2 && cpufreq_lcd_status == 0) {
                   cpu_down(1);
		    }
...
static int cpufreq_interactiveplus_notifier(
...
static struct attribute *interactiveplus_attributes[] = {
...
static struct attribute_group interactiveplus_attr_group = {
	.attrs = interactiveplus_attributes,
	.name = "interactiveplus",
};
...
{
	cpufreq_lcd_status = 0;
	current_timer_rate = timer_rate;
	timer_rate = DEFAULT_TIMER_RATE * 10;
}

static void cpufreq_late_resume(struct early_suspend *h)
{
	cpufreq_lcd_status = 1;
	timer_rate = current_timer_rate;
}
...
static int __init cpufreq_interactiveplus_init(void)
{
	...
	mutex_init(&gov_lock);
	speedchange_task =
		kthread_create(cpufreq_interactiveplus_speedchange_task, NULL,
			       "cfinteractiveplus");
    ...
    cpufreq_lcd_status = 1;
    early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 25;
